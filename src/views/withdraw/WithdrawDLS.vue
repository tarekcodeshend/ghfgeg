<template>
    <div class="deposit-robux">
        ...
        <div class="robux-withdraw-amount-title">Withdraw amount</div>
        <div class="robux-amount">
            <input v-model="modalAmount" v-on:input="modalValidateInput()" type="text"
                placeholder="0.00" />
        ...
        <div class="robux-info">
            <div class="info-deposit">
                Please note that the minimum withdrawal amount is 0.20 Goonbux and the maximum withdrawal amount is 15 BGL.
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        modalActionButton() {
            ...
            if (amount === null || isNaN(amount) === true || amount <= 0) {
                this.notificationShow({ type: 'error', message: 'Your entered withdraw amount is invalid.' });
                return;
            }

            const data = { amount: amount };
            this.cashierSendRobuxWithdrawSocket(data);
            this.modalAmount = null;
        },
    },
    computed: {
    ...
    created() {
        ...
        if (this.cashierRobuxData.loading === false) {
            const data = { page: this.cashierRobuxData.page, type: 'withdraw' };
