<template>
    <div class="login-roblox">
        ...
        <div class="roblox-info">
            By accessing STARFlip.live, I attest that I am at least 18 years old and have read and agree with the <button
                v-on:click="loginTermsButton()" class="button-terms">Terms of Service</button>.
        </div>
        <div class="roblox-buttons">
            <button v-on:click="$parent.modalSetTab(tab === 'roblox login' ? 'roblox cookie' : 'roblox login')"
                class="button-toggle" v-bind:class="{ 'button-active': tab === 'roblox cookie' }"
                v-bind:disabled="authSendLoginLoading === true || authStep !== 'login'">
                <div class="button-inner">.ROBLOSECURITY</div>
            </button>
            <button v-on:click="loginActionButton()" class="button-action"
...
.login-roblox button.button-toggle.button-active {
    filter: drop-shadow(0px 4px 25px rgba(1, 230, 169, 0.15)) drop-shadow(0px 2px 25px rgba(15, 41, 63, 0.35));
}
...
.login-roblox button.button-action .button-inner {
    color: #ffffff;
    background: linear-gradient(250deg, #ffdd00 0%, #00aa6d 100%);
}
