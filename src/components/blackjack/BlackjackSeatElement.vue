<template>
    <div class="blackjack-seat-element">
        <div class="element-user">
            <button v-if="player === undefined" v-on:click="$parent.blackjackJoinButton(seat)" class="button-join">Join
                {{ seat + 1 }}</button>
        ...
        <div class="element-game" v-bind:class="{ 'game-completed': table.game.state === 'completed' }">
            <div v-if="['running', 'completed'].includes(table.game.state) === true && player !== undefined"
                class="game-stack">
                ...
                <div v-if="player.bet.actions.includes('split') === false" class="stack-single">
                    <transition-group name="player" tag="div" class="single-cards">
                        <BlackjackCard v-for="(card, index) of player.bet.cards"
                            v-bind:key="index + card.suit + card.rank" v-bind:card="card" />
                    </transition-group>
                    <BlackjackValue v-bind:value="blackjackGetCardValue(player.bet.cards)"
                        v-bind:state="blackjackGetCardState(player.bet.cards)" v-bind:class="{
                ...
                <div v-else class="stack-split">
                    <div class="split-left">
                        <transition-group name="player" tag="div" class="left-cards">
                            <BlackjackCard v-for="(card, index) of player.bet.cardsLeft"
                                v-bind:key="index + card.suit + card.rank" v-bind:card="card" />
                        </transition-group>
                        <BlackjackValue v-bind:value="blackjackGetCardValue(player.bet.cardsLeft)"
                            v-bind:state="blackjackGetCardState(player.bet.cardsLeft)" v-bind:class="{
                    ...
                    <div class="split-right">
                        <transition-group name="player" tag="div" class="right-cards">
                            <BlackjackCard v-for="(card, index) of player.bet.cardsRight"
                                v-bind:key="index + card.suit + card.rank" v-bind:card="card" />
                        </transition-group>
                        <BlackjackValue v-bind:value="blackjackGetCardValue(player.bet.cardsRight)"
                            v-bind:state="blackjackGetCardState(player.bet.cardsRight)" v-bind:class="{
            ...
            <div class="game-actions">
                <button v-on:click="$parent.blackjackBetButton(seat, 'sideLeft')" class="button-side">
                    <transition name="slide-fade" mode="in-out">
                        <div v-if="blackjackGetAmount.sideLeft >= (100 * 1000)" class="button-chip" key="orange-chip">
                            ...
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.sideLeft / 1000) }}
                            </div>
                        </div>
                        ...
                        <div v-else-if="blackjackGetAmount.sideLeft >= (1 * 1000)" class="button-chip"
                            key="purple-chip">
                ...
                <button v-if="this.table.game.type === 'standard'" v-on:click="$parent.blackjackBetButton(seat, 'main')"
                    class="button-main">
                    <transition name="slide-fade" mode="in-out">
                        <div v-if="blackjackGetAmount.main >= (2.5 * 1000)" class="button-chip" key="orange-chip">
                            <img src="@/assets/img/blackjack/chip-orange.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.main / 1000) }}</div>
                        </div>
                        <div v-else-if="blackjackGetAmount.main >= (1 * 1000)" class="button-chip" key="blue-chip">
                            <img src="@/assets/img/blackjack/chip-blue.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.main / 1000) }}</div>
                        </div>
                        <div v-else-if="blackjackGetAmount.main >= (0.5 * 1000)" class="button-chip" key="purple-chip">
                            <img src="@/assets/img/blackjack/chip-purple.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.main / 1000) }}</div>
                        </div>
                        <div v-else-if="blackjackGetAmount.main >= (0.25 * 1000)" class="button-chip" key="red-chip">
                            <img src="@/assets/img/blackjack/chip-red.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.main / 1000) }}</div>
                        </div>
                    </transition>
                </button>
                <button v-else v-on:click="$parent.blackjackBetButton(seat, 'main')" class="button-main">
                    <transition name="slide-fade" mode="in-out">
                        <div v-if="blackjackGetAmount.main >= (50 * 1000)" class="button-chip" key="orange-chip">
                            <img src="@/assets/img/blackjack/chip-orange.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.main / 1000) }}</div>
                        </div>
                        <div v-else-if="blackjackGetAmount.main >= (25 * 1000)" class="button-chip" key="blue-chip">
                            <img src="@/assets/img/blackjack/chip-blue.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.main / 1000) }}</div>
                        </div>
                        <div v-else-if="blackjackGetAmount.main >= (10 * 1000)" class="button-chip" key="purple-chip">
                            <img src="@/assets/img/blackjack/chip-purple.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.main / 1000) }}</div>
                        </div>
                        <div v-else-if="blackjackGetAmount.main >= (2.5 * 1000)" class="button-chip" key="red-chip">
                            <img src="@/assets/img/blackjack/chip-red.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.main / 1000) }}</div>
                        </div>
                    </transition>
                ...
                <button v-on:click="$parent.blackjackBetButton(seat, 'sideRight')" class="button-side">
                    <transition name="slide-fade" mode="in-out">
                        <div v-if="blackjackGetAmount.sideRight >= (100 * 1000)" class="button-chip" key="orange-chip">
                            <img src="@/assets/img/blackjack/chip-orange.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.sideRight / 1000) }}
                            </div>
                        </div>
                        <div v-else-if="blackjackGetAmount.sideRight >= (10 * 1000)" class="button-chip"
                            key="blue-chip">
                            <img src="@/assets/img/blackjack/chip-blue.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.sideRight / 1000) }}
                            </div>
                        </div>
                        <div v-else-if="blackjackGetAmount.sideRight >= (1 * 1000)" class="button-chip"
                            key="purple-chip">
                            <img src="@/assets/img/blackjack/chip-purple.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.sideRight / 1000) }}
                            </div>
                        </div>
                        <div v-else-if="blackjackGetAmount.sideRight >= (0.1 * 1000)" class="button-chip"
                            key="red-chip">
                            <img src="@/assets/img/blackjack/chip-red.png" />
                            <div class="chip-amount">{{ blackjackFormatAmount(blackjackGetAmount.sideRight / 1000) }}
                            </div>
