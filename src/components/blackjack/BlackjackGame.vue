    ...
    <div class="blackjack-game">
        ...
        <div v-if="['running', 'completed'].includes(table.game.state) === true" class="game-dealer">
            <transition-group name="dealer" tag="div" class="dealer-cards">
                <BlackjackCard v-for="(card, index) of table.game.dealerCards"
                    v-bind:key="index + card.suit + card.rank" v-bind:card="card" />
            </transition-group>
            <BlackjackValue v-bind:value="blackjackGetCardValue(table.game.dealerCards)" state="" v-bind:class="{
        ...
        <div class="game-seats">
            ...
            <BlackjackSeatElement v-for="index in 5" v-bind:key="index" v-bind:table="table" v-bind:seat="index - 1"
...
export default {
    ...
    methods: {
        ...
        blackjackGetAmount(chip) {
            const amounts = this.table.game.type === 'standard' ? [0.25, 0.5, 1, 2.5] : [2.5, 10, 25, 50];
            return amounts[chip];
        },
