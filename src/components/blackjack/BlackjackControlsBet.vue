    ...
    <div class="blackjack-controls-bet">
        ...
        <div class="bet-container">
            <div class="container-amount">
                <button v-on:click="$parent.blackjackSetChip(0)" class="button-red"
                    v-bind:class="{ 'button-active': blackjackChip === 0 }">{{ table.game.type === 'standard' ? '0.25' :
                        '2.5' }}</button>
                <button v-on:click="$parent.blackjackSetChip(1)" class="button-purple"
                    v-bind:class="{ 'button-active': blackjackChip === 1 }">{{ table.game.type === 'standard' ? '0.5' :
                        '10' }}</button>
                <button v-on:click="$parent.blackjackSetChip(2)" class="button-blue"
                    v-bind:class="{ 'button-active': blackjackChip === 2 }">{{ table.game.type === 'standard' ? '1' :
                        '25' }}</button>
                <button v-on:click="$parent.blackjackSetChip(3)" class="button-orange"
                    v-bind:class="{ 'button-active': blackjackChip === 3 }">{{ table.game.type === 'standard' ? '2.5' :
                        '50' }}</button>
            </div>
            <div class="container-buttons">
                <button v-on:click="blackjackClearButton()" class="button-clear"
                    v-bind:disabled="socketSendLoading !== null">
                    <div class="button-inner">CLEAR</div>
                </button>
                <button v-on:click="blackjackRedoButton()" class="button-redo"
                    v-bind:disabled="socketSendLoading !== null">
                    <div class="button-inner">REDO BET</div>
                </button>
                <button v-on:click="blackjackMaxButton()" class="button-max"
                    v-bind:disabled="socketSendLoading !== null">
    ...
    methods: {
        ...
        blackjackMaxButton() {
            ...
            for (const player of this.table.players.filter((element) => element.user._id === this.authUser.user._id)) {
                ...
                const amountBet = Math.floor(((this.table.game.type === 'standard' ? 500 : 1000) * 1000) - amountCurrent);
