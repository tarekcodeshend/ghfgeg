<template>
    <div class="crash-controls">
        ...
        <div class="controls-amount">
            <input v-model="crashAmount" v-on:input="crashValidateInput" v-on:change="crashFormatInput" type="text"
                placeholder="BET AMOUNT" v-bind:disabled="crashAutoRunning === true" />
            ...
            <div class="cashout-buttons">
                ...
                <button v-on:click="crashSetInput('crashAmount', 'max')" class="button-max"
                    v-bind:disabled="crashAutoRunning === true">
                    <div class="button-inner">MAX</div>
                </button>
            </div>
        </div>
        <div class="controls-cashout-title">Multiplier</div>
        <div class="controls-cashout">
            <input v-model="crashAutoCashout" v-on:input="crashValidateInputCashout"
                v-on:change="crashFormatInputCashout" type="text" placeholder="AUTO CASHOUT"
                v-bind:disabled="crashAutoRunning === true" />
            <div class="cashout-buttons">
                <button v-on:click="crashSetInput('crashAutoCashout', 'increase')"
                    v-bind:disabled="crashAutoRunning === true">
                    <img src="@/assets/img/iconup.png" alt="up" />
                </button>
                <button v-on:click="crashSetInput('crashAutoCashout', 'decrease')"
                    v-bind:disabled="crashAutoRunning === true">
                ...
                <div class="bets-header">
                    ...
                    <div class="header-amount">
                        ...
                        <div class="amount-value">
                            <span>{{ crashFormatValue(crashGetBetsAmount).split('.')[0] }}</span>.{{
                                crashFormatValue(crashGetBetsAmount).split('.')[1] }}
                        </div>
                    </div>
                </div>
        ...
        <div v-else class="controls-auto">
            <div class="auto-adjust">
                <div class="adjust-win">
                    <input v-model="crashAutoPercentageWin" type="text" placeholder="% ON WIN"
                        v-bind:disabled="crashAutoRunning === true" />
                </div>
                <div class="adjust-lose">
                    <input v-model="crashAutoPercentageLoss" type="text" placeholder="% ON LOSS"
                        v-bind:disabled="crashAutoRunning === true" />
                </div>
            </div>
            <div class="auto-stop">
                <div class="stop-profit">
                    <input v-model="crashAutoStopProfit" type="text" placeholder="STOP ON PROFIT"
                        v-bind:disabled="crashAutoRunning === true" />
                    <img src="@/assets/img/coins.8ad8b473.png" alt="icon" />
                </div>
                <div class="stop-lose">
                    <input v-model="crashAutoStopLoss" type="text" placeholder="STOP ON LOSS"
                        v-bind:disabled="crashAutoRunning === true" />
                    <img src="@/assets/img/coins.8ad8b473.png" alt="icon" />
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        crashValidateInputCashout() {
            this.crashAutoCashout = this.crashAutoCashout.replace(',', '.').replace(/[^\d.]/g, '');
            this.crashAutoCashout = this.crashAutoCashout.indexOf('.') >= 0 ? this.crashAutoCashout.substr(0, this.crashAutoCashout.indexOf('.')) + '.' + this.crashAutoCashout.substr((this.crashAutoCashout.indexOf('.') + 1), 2).replace('.', '') : this.crashAutoCashout;
        },
        crashFormatInput() {
        ...
        crashSetInput(value, action) {
            ...
            else if (amount >= Math.floor(this.authUser.user.balance)) {
...
.crash-controls {
    ...
    padding: 15px 10px;
    border-radius: 15px;
    background: linear-gradient(180deg, #241D2E 0%, #231c2e 74%, #1a1422 100%);
}
...
.crash-controls .controls-mode button.button-active .button-inner span {
    background: linear-gradient(255deg, #ffdd00 0%, #00aa6d 100%);
...
.crash-controls .cashout-buttons button {
    ...
    height: 37px;
    border-radius: 7px;
    background: linear-gradient(302deg, rgba(255, 255, 255, 0.00) 50.11%, rgba(255, 255, 255, 0.00) 100%), #161b23;
    background-blend-mode: plus-lighter, normal;
    transition: all .2s;
}

.crash-controls .cashout-buttons button img {
    width: 18px;
    height: 18px;
    margin-top: 4px;
}

.crash-controls .cashout-buttons button:first-of-type img {
    width: 18px;
    height: 18px;
    margin-top: 3px;
}
...
.crash-controls .controls-manual button.button-cashout .button-inner {
    ...
    color: #fff;
    background: #0089ff;
    border-bottom: 2px solid #b39029;
    transition: all 0.3s ease;
}
...
.crash-controls .controls-manual button.button-bet .button-inner:hover {
    background: #00de56;
}

.crash-controls .controls-manual button.button-cashout .button-inner:hover {
    background: #dbba56;
}
