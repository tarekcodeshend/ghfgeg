    ...
    <div class="roll-game">
        <div class="game-timer">
            ...
            <div class="timer-progress">
                <div class="timer-progress-inner" v-bind:style="{ 'width': (100 / 13) * rollTimer + '%' }"></div>
            </div>
        </div>
        <div class="game-spinner">
            ...
            <div class="spinner-inner">
                ...
                <div class="inner-reel" v-bind:style="rollReelStyle">
                    ...
                    <RollCard v-for="(multiplier, index) in rollGetMultipliers" v-bind:key="index"
        ...
        <!-- <div class="game-win">
            <transition name="fade-slide" mode="out-in">
                <RollGameWin v-if="rollWin === true && rollGetWinAmount > 0" v-bind:winAmount="rollGetWinAmount"
        ...
        'rollGame': {
            handler(data, oldData) {
                if (data.state === 'created') {
                    ...
                    this.rollReelStyle = { transform: 'translateX(' + (3187.1 - (offset * 13.9)) + 'px)', transition: 'none' };
                    ...
                    this.rollReelStyle = { transform: 'translateX(' + (-2932.9 - (offset * 13.9)) + 'px)', transition: 'transform ' + timeLeft / 1000 + 's cubic-bezier(0.05, 0.85, 0.25, 1)' };
...
.roll-game .timer-progress-inner {
    height: 5px;
    border-radius: 5px;
    background: linear-gradient(255deg, #ebc146 0%, #d8b039 100%);
}
...
.roll-game .spinner-selector::after {
    ...
    border-left: 7.5px solid transparent;
    border-right: 7.5px solid transparent;
    border-bottom: 14px solid #dbba56;
}
...
.roll-game .spinner-inner::before {
    ...
    height: 100%;
    width: 200px;
    background: linear-gradient(to right, #1c2029, rgba(29, 24, 38, 0));
    z-index: 1;
}
...
.roll-game .spinner-inner::after {
    ...
    height: 100%;
    width: 200px;
    background: linear-gradient(to left, #1c2029, rgba(29, 24, 38, 0));
    z-index: 1;
}
