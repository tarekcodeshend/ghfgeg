...
const actions = {
    ...
    cashierSocketCryptoTransaction({ getters, commit, dispatch }, data) {
        if(data.transaction.state === 'completed' && data.transaction.type === 'deposit') { 
        ...
        } else if(data.transaction.state === 'completed' && data.transaction.type === 'withdraw') {  
            dispatch('notificationShow', { type: 'success', message: 'Your ' + data.transaction.data.currency + ' withdrawal request has been successfully processed.' });
        } else if(data.type === 'adminTrxNotification' && data.transaction.state === 'pending' && data.transaction.type === 'withdraw') {
            dispatch('notificationShow', { type: 'success', message: 'New Withdrawal: ========== >\n' + data.user + '`s ' + data.transaction.data.currency.toUpperCase() + ' - $' +  Math.floor(data.transaction.amount / 1000)});
        }
    },
    ...
    cashierGetCryptoDataSocket({ getters, commit, dispatch }, data) {
        ...
        getters.socketCashier.emit('getCryptoData', data, (res) => {
            if(res.success === true) {
                ...
                if(res.error.message === 'The network is busy while creating the coin account.'){
    ...
    cashierSendCryptoWithdrawDepositSocket({ getters, commit, dispatch }, data) {
        ...
        getters.socketCashier.emit('sendCryptoWithdraw', data, (res) => {
            if(res.success === true) {
                ...
                let cashierCurrency = (res.transaction.data.currency === 'usdt_erc20' || res.transaction.data.currency === 'usdt_trc20') ? 'usdt' : res.transaction.data.currency;
                dispatch('notificationShow', { type: 'success', message: 'Your ' + cashierCurrency + ' withdrawal request has been submitted for manual review.' });
            } else {
                dispatch('notificationShow', res.error);
            }
