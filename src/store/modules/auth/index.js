...
const actions = {
    ...
    async authSendCredentialsUsername({ getters, commit, dispatch }, data) {
        if(getters.authSendLoginLoading === true) { return; }
        commit('auth_set_send_login_loading', true);

        try {
            const res = await axios.post('/auth/credentials/username', data);
            if(res.data.success) {
                commit('auth_set_user', { ...getters.authUser.user, username: res.data.user.username });
                dispatch('notificationShow', { type: 'success', message: 'Your username has been updated successfully.' });
            }
        } catch(err) {
            if(err.response !== undefined && err.response !== null) {
                dispatch('notificationShow', err.response.data.error);
