...
const mutations = {
    ...
    user_set_seed_data(state, data) {
        state.userSeedData.data = { seed: data.seed };
    },
...
const actions = {
    ...
    userSendServerSeedSocket({ getters, commit, dispatch }, data) {
        if(getters.socketGeneral === null || getters.socketSendLoading !== null) { return; }
        commit('socket_set_send_loading', 'UserServerSeed');

        getters.socketGeneral.emit('sendUserServerSeed', data, (res) => {
            if(res.success === true) {
                commit('user_set_seed_data', res);
